...
.486
.model flat, stdcall						
option casemap :none					 ; ?oanoaeoaeuiinou e ?aaeno?o aoea a eaaioeoeeaoi?ao
include windows.inc
...
entryPoint:
;-----------------------------------------------------------------------------------------------------;	
		push STD_INPUT_HANDLE					    ;iioie aaiaa 
		call GetStdHandle					
		mov inputHandle, EAX						 
	
		push STD_OUTPUT_HANDLE					   ;iioie auaiaa
		call GetStdHandle       
...
;-----------------------------------------------------------------------------------------------------;	  
	...
	mov EBX, offset inputBuffer1
        ...
        mov byte ptr [ EBX + EAX - 1 ], " "			;aiaaaeyai a eiioa no?iee neaaie " "aey ia?aaiaa eo?ni?a 
	...
	push 0
	call ExitProcess					 ;caaa?oaiey i?eei?aiey
